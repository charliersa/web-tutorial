...
const TWEAKS_STYLE = `
  ...
  .twk-fab:hover{background:rgba(0,0,0,.82)}
`;
...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  // FAB button offset needs to shift up when panel is closed
  const fabBottom = open ? 'calc(16px + 280px + 8px)' : '16px';
  ...
  return (
    <>
      ...
      <button
        ...
        title="開啟視覺設定 (T)"
        onClick={() => setOpen((v) => !v)}
        style={{ bottom: open ? 'calc(16px + 36px + 8px)' : '16px' }}
      >
        ⚙
      </button>
      {open && (
